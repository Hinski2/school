...
(* zmiana  | Block ss    -> List.iter (eval_stmt env) ss*)
let rec eval_stmt env s =
  match s with
  | Block (vs, ss) -> let local_env =
      List.fold_left (fun acc v -> declare_var acc v) env vs in
      List.iter (eval_stmt local_env) ss
