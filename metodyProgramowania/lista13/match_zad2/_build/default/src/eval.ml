...
and value =
  ...
  | VCtor of cname * value list (* zmiana na value list*)
...
let rec match_pattern env v p =
  ...
  | VCtor(c1, vs), PCtor(c2, ps) when c1 = c2 -> (*zmiana w VCtor*)
    match_VCtor env vs ps
  | _, PCtor _ -> None
...
and match_VCtor env vs ps = 
  match vs, ps with
  | a::b, c::d -> 
    let new_env = match_pattern env a c in
    (match new_env with
    | Some new_env -> match_VCtor new_env b d
    | None -> None)
  | [], [] -> Some env
  | _, _ -> None
...
let check_pattern p =
  let rec aux p ids = 
    ...
    | PPair(p1, p2) -> 
      ...
      aux p2 new_ids
    | PCtor(_, []) -> ids
    | PCtor(c, h::t) -> aux (PCtor(c, t)) (aux h ids) 
    | PAs(_, id) -> 
      if List.mem id ids
      then failwith ("duplicate id " ^ id)
      else id::ids
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Int n -> VInt n
  | Bool b -> VBool b
  | Ctor(c, es) -> evalCtor env c es [] (* dodanie funkcji *)
  | If (p, t, e) ->
...
and evalCtor env c es acc =
  match es with
  | [] -> VCtor(c, acc)
  | h::t -> evalCtor env c t ((eval_env env h) :: acc)
...
and print_VCtor (c, vs) = 
  let rec join_rec_vs vs =
    match vs with
    | [] -> ""
    | [v] -> string_of_value v  (* tak by nie stawiać ',' na końcu*)
    | h::t -> (string_of_value h) ^ ", " ^ (join_rec_vs t) in

  match vs with
  | [] -> c ^ "()"        (* w konstruktorze nie mamy elementów *)
  | [v] -> (match v with  (* w konstrukorze tylko jeden element *)
