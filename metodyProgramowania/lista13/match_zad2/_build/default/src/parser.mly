...
%token TRUE FALSE IF THEN ELSE LET IN AS
...
patterns:       
  | p = pattern; ps = patterns { p :: ps }
  | p = pattern { [p] }

mixfix:
  | IF; e1 = mixfix; THEN; e2 = mixfix; ELSE; e3 = mixfix { If(e1, e2, e3) }
  | LET; x = pattern; EQ; e1 = mixfix; IN; e2 = mixfix { Let(x, e1, e2) }
  | FUN; ps = patterns; ARR; e = mixfix { make_funs ps e }  
...
app:
  ...
  | SND; e = base { Snd e }
  | RAISE { Raise }
  | c = CNAME; es = bases { Ctor(c, es) } (* zmiana Ctor *)
  | e = base { e }
  ;

bases: (* dodanie *)
  | b=base; bs=bases { b::bs }
  | b=base { [b] }
...
pattern:
  ...
  | p=pattern_app; AS; x=IDENT { PAs(p, x) }
  | p=pattern_app { p }
  ;

pattern_app:
  | c = CNAME; ps = pattern_bases { PCtor(c, ps) }  (* zmiana PCtor *)
  | p=pattern_base { p }
  ;

pattern_bases:  (* dodanie pattern_bases*)
  | pb=pattern_base; COMMA; pbs=pattern_bases { pb::pbs }
  | pb=pattern_base { [pb] }
