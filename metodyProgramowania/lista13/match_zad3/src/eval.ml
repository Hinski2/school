...
let rec match_pattern env v p =
  ...
  | _, PAs(p, x) -> 
    ...
    | None -> None)
  | _, PWhen(p, e) ->                 (*dodanie PWhen*)
    (match match_pattern env v p with
    | Some env -> if ((eval_env env e = VBool(true))) (*sprawdzenie czy expr oblicza się do True w nowym środowisku*)
      then Some env
      else None
    | None -> None)
...
and check_pattern p =
  let rec aux p ids = 
    ...
    | PWhen(pat, _) -> aux pat ids (*dodanie PWhen*)
    in ignore(aux p [])
