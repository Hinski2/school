...
and value =
  ...
  | VInt of int
  | VBool of bool
  | VClosure of pattern * expr * env (*zmiana ident na pattern*)
...
let rec eval_env (env : env) (e : expr) : value =
  ...
  | Let (pat, e1, e2) ->
      ...
      let new_env = add_pattern env r pat in    (*tutaj dodaje funcje do dodawania patternu do env*)
      eval_env new_env e2
  | Var x ->
  ...
  | App (e1, e2) ->
      ...
      | VClosure (pat, body, clo_env), v -> eval_env (add_pattern clo_env v pat) body (*tutaj teżtrzeba dodać tą funkcje*)
