...
static int readnum(int *num_p) {
  ...
  int signo = sigsetjmp(env, 1); // 1 - zapisz obocną maske sygnałów
  if(signo) // jeśli wracamy z sygnałem
    return signo;

  alarm(1); // uruchamiamy alarm by odliczać timeout
  n = Read(STDIN_FILENO, line, MAXLINE - 1);
  if(n <= 0) return 0;


  line[n] = '\0';
  alarm(0); // wyłączamy czasomież, bo user wpisał input
...
static void game(void) {
  ...
  while (lives > 0) {
    switch (last_sig) {
      ...
      case SIGINT:  // kończymy gre
        printf(CHA(1) EL() "Bye bye!\n");
        exit(EXIT_FAILURE);

      case SIGALRM: // tracimy życie
        timeout--;
        if (timeout < 0) {
          last_sig = 0;
          lives--;
