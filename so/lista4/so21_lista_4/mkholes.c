...
int main(void) {
  ...
  for (int i = 0; i < NBLKS; i++) {
    ...
    if (random() % 64) { // prawdopodobieństwo 63/64 że robimy dziure
      Lseek(fd, BLKSZ, SEEK_CUR);
      Write(STDERR_FILENO, ".", 1); // na stder wypisujemy .
    } else { // prawdopodobieństwo 1/64 że robiimy zapsi do poliku randomowych liczb
      char blk[BLKSZ];
      for (int j = 0; j < BLKSZ; j++)
        blk[j] = random();
      Write(fd, blk, BLKSZ);
      Write(STDERR_FILENO, "O", 1); // na stdr wypisujey O
      usedblks++;
    }
    if (i % WRAP == WRAP - 1)
      Write(STDERR_FILENO, "\n", 1);
