...
static void print_mode(mode_t m) {
  ...
  char ur = (m & S_IRUSR) ? 'r' : '-';  // user(owner) read
  char uw = (m & S_IWUSR) ? 'w' : '-';
  char ux = (m & S_IXUSR) ? 'x' : '-';
  char gr = (m & S_IRGRP) ? 'r' : '-';
  char gw = (m & S_IWGRP) ? 'w' : '-';  // group write
  char gx = (m & S_IXGRP) ? 'x' : '-';
  char or = (m & S_IROTH) ? 'r' : '-';  
  char ow = (m & S_IWOTH) ? 'w' : '-';
  char ox = (m & S_IXOTH) ? 'x' : '-';  // others execute

  /* TODO: Fix code to report set-uid/set-gid/sticky bit as 'ls' does. */
  if(m & S_ISUID) ux = (ux == 'x') ? 's' : 'S'; // proces(uruchomony z tego pliku) dostaje uid właścieciela pliku
  if(m & S_ISGID) gx = (gx == 'x') ? 's' : 'S'; // proces dostaje gid grupy pliku
  if(m & S_ISVTX) ox = (ox == 'x') ? 't' : 'T'; // dla katalogów. pozwala usuwać/przenosic pliki w katalogo tylko ich właścicielowi lub rootowi
...
static void file_info(int dirfd, const char *name) {
  ...
  if(S_ISCHR(sb -> st_mode) || S_ISBLK(sb -> st_mode)) {
    printf(" %u, %u", major(sb -> st_rdev), minor(sb -> st_rdev)); // major identyfikujesterownik, minor konkretną instancje urządzenia
  } else {
  ...
  /* TODO: Read where symlink points to and print '-> destination' string. */
    ...
    size_t k = readlinkat(dirfd, name, linkbuf, PATH_MAX);
    linkbuf[k] = '\0';
    printf(" -> %s", linkbuf);
  }
...
int main(int argc, char *argv[]) {
  ...
  while ((n = Getdents(dirfd, (void *)buf, DIRBUFSZ))) {
    ...
    for(size_t bpos = 0; bpos < n; bpos += d -> d_reclen) {
