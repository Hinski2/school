...
def find_ans(matrix, rows_poss, cols_poss, rows_done, cols_done, rows, cols):
    ...
    while not solved and change: 
        ...
        for ammount, i, enum in lines_todo:
            if ammount == 0:
                give_back_removed_poss(len(matrix), len(matrix[0]), removed_poss_rows, removed_poss_cols, rows_poss, cols_poss)
                return []
    ...
    for u in to_check:
        ...
        if check_solved(_rows_done, _cols_done):
            if valid(_matrix, rows, cols): return _matrix
                ...
                give_back_removed_poss(len(matrix), len(matrix[0]), removed_poss_rows2, removed_poss_cols2, rows_poss, cols_poss)
                continue
