...
def find_ans(matrix, rows_poss, cols_poss, rows_done, cols_done, rows, cols):
    ...
    removed_poss_rows = [[] for _ in range(len(matrix))]
    removed_poss_cols = [[] for _ in range(len(matrix[0]))]
    ...
    for j in range(len(rows_poss)):
        if len(rows_poss[j]) == 0 and not rows_done[j]:
            give_back_removed_poss(enum, len(matrix), len(matrix[0]), removed_poss_rows, removed_poss_cols, rows_poss, cols_poss)
            return []
    ...
    for u in to_check:
        ...
        removed_poss_rows2 = [[] for _ in range(len(matrix))]
        removed_poss_cols2 = [[] for _ in range(len(matrix[0]))]
        ...
        ans = find_ans(_matrix, rows_poss, cols_poss, _rows_done, _cols_done, rows, cols)
        if ans != []:
            return ans
        
        # dodanie opcji które wcześniej usuneliśmy
        give_back_removed_poss(enum, len(matrix), len(matrix[0]), removed_poss_rows2, removed_poss_cols2, rows_poss, cols_poss)
