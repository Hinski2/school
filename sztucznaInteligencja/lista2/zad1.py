...
def row_opt_dist(row, desc):
    l, r = 0, len(row)
    left = sum(desc) + len(desc) #ile bloków musze zachowac na potem 
    ans = 0
    blocks = [0 for _ in range(r)]
    
    # ustawiam każdy z bloków
    for blok in desc:
        left -= blok + 1
        ...
        # przesówam lewy koniec 
        l += blok + 1
    
    #sprawdzam ile bloków się niezgadza
    for i in range(r):
        if row[i] != blocks[i]:
            ans += 1
...
def col_opt_dist(nanogram, col_desc, col):
    return row_opt_dist(get_col(nanogram, col), col_desc[col])
...
def make_nanogram(n, m, row_desc, col_desc):
    ...
    while True:
        ...
        # tworzenie col_todo
        for i in range(m):
            if row_opt_dist(get_col(nanogram, i), col_desc[i]) > 0:
                col_todo.append(i)
        
        # tworzenie row_todo
        for i in range(n):
            if row_opt_dist(nanogram[i], row_desc[i]) > 0:
                row_todo.append(i)
        ...
        for itr in range(limit):
            ...
            if choice == 0:
                ...
                for row in range(n):
                    row_diff = row_opt_dist(nanogram[row], row_desc[row])
                    diff = col_diff + row_diff
                    
                    # sprawdzam jak dużo da mi zmiana piksela
                    change_pixel(nanogram, row, col)
                    diff -= col_opt_dist(nanogram, col_desc, col) + row_opt_dist(nanogram[row], row_desc[row])
                    change_pixel(nanogram, row, col)
            ...
            res = row_opt_dist(nanogram[to_change_row], row_desc[to_change_row])
